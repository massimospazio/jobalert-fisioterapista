...
def _homecare(text: str) -> bool:
    lowered = text.lower()
    return bool(re.search(r"\badi\b", lowered)) or any(token in lowered for token in ["domiciliar", "a domicilio", "assistenza domiciliare"])
...
def _homecare_only(title: str, text: str) -> bool:
    ...
    if any(token in combined for token in ["ambulator", "poliambulator", "studio", "clinica", "rsa", "ospedal", "reparto"]):
        return False
    return "domiciliar" in title.lower() or bool(re.search(r"\badi\b", title.lower())) or any(token in combined for token in ["assistenza domiciliare", "servizio adi", "pazienti a domicilio", "prestazioni domiciliari"])
...
def _published(text: str):
    ...
    if re.search(r"(\d+)\s+(?:ora|ore)\s+fa", lowered):
        return date.today().isoformat()
    return None


def _extract_cards(soup: BeautifulSoup):
    for selector in ["div.job_seen_beacon", "div.cardOutline", "li div.job_seen_beacon", "div.slider_container div.cardOutline"]:
...
def collect(source_config: dict, locations: dict) -> tuple[list[JobListing], dict]:
    ...
    params = {"url": search_url, "apikey": api_key, "js_render": "true", "premium_proxy": "true"}
    response = requests.get(ZENROWS_ENDPOINT, params=params, timeout=120)
    html = response.text or ""
    usage = {"status": response.status_code, "request_cost": response.headers.get("X-Request-Cost", "n/a"), "credits": 25, "request_id": response.headers.get("X-Request-Id", "n/a"), "bytes": len(response.content)}
    print(f"INDEED_ZENROWS_USAGE request_cost={usage['request_cost']} credits=25 status={usage['status']} bytes={usage['bytes']} request_id={usage['request_id']}")
    if response.status_code >= 400:
        raise RuntimeError(f"ZenRows Indeed HTTP {response.status_code}")
    record_success("Indeed", credits=25, request_cost=response.headers.get("X-Request-Cost"), request_id=response.headers.get("X-Request-Id"))

    soup = BeautifulSoup(html, "lxml")
    cards = _extract_cards(soup)
    jobs, seen = [], set()
    missing_anchor = missing_jk = 0
    for card in cards:
        ...
        if not identity or identity in seen:
            continue
        company_node = card.select_one('[data-testid="company-name"], span.companyName, .company_location [data-testid="company-name"], .company_location span:first-child')
        ...
        if len(jobs) >= limit:
            break
    print(f"INDEED_COLLECT cards={len(cards)} jobs={len(jobs)} missing_anchor={missing_anchor} missing_jk={missing_jk}")
    if not cards:
